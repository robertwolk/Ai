...
const navItems = [
  ...
  { type: "divider" as const, label: "Analytics" },
  {
    label: "Reports",
    href: "/reports",
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 h-screen w-64 bg-sidebar-bg text-sidebar-fg flex flex-col">
      ...
      <div className="flex h-16 items-center gap-2 px-6 border-b border-white/10">
        ...
        <span className="text-lg font-bold">CRM Hub</span>
      </div>
          ...
          if ("type" in item && item.type === "divider") {
            return (
              <div key={i} className="pt-4 pb-2 px-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-sidebar-fg/40">
                  {item.label}
                </span>
              </div>
            );
